...
{
	if (opcode == 0xC8 && ctx->level >= SX86_CPULEVEL_8086) {	// ENTER
		sx86_uword Size;
	...
	if (opcode == 0xC9 && ctx->level >= SX86_CPULEVEL_8086) { // LEAVE
...
{
	if (opcode == 0xC8 && ctx->level >= SX86_CPULEVEL_8086) {
		sx86_uword popeye;
		sx86_ubyte imm;
	...
	if (opcode == 0xC9 && ctx->level >= SX86_CPULEVEL_8086) {
		strcpy(buf,"LEAVE");
		return 1;
	}
