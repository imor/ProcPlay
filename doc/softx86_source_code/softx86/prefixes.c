...
{
	if (opcode == 0x26) {		// ES:
		ctx->is_segment_override = 1;
		ctx->segment_override = ctx->state.segment_reg[SX86_SREG_ES].val;
		return 2;
	}
	if (opcode == 0x2E) {		// CS:
		ctx->is_segment_override = 1;
		ctx->segment_override = ctx->state.segment_reg[SX86_SREG_CS].val;
		return 2;
	}
	if (opcode == 0x36) {		// SS:
		ctx->is_segment_override = 1;
		ctx->segment_override = ctx->state.segment_reg[SX86_SREG_SS].val;
		return 2;
	}
	if (opcode == 0x3E) {		// DS:
		ctx->is_segment_override = 1;
		ctx->segment_override = ctx->state.segment_reg[SX86_SREG_DS].val;
		return 2;
	}
