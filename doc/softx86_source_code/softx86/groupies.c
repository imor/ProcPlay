...
int Sfx86OpcodeExec_group80(sx86_ubyte opcode,softx86_ctx* ctx)
{
	if ((opcode&0xFC) == 0x80) {
		...
		if (reg == 7)			/* CMP doesn't modify the dest. register... */
			sx86_exec_full_modrmonly_ro_imm(ctx,w16,0,mod,rm,op8,op16,op32);
		else
			sx86_exec_full_modrmonly_rw_imm(ctx,w16,0,mod,rm,op8,op16,op32);
...
/* TEST does not modify the dest. register... */
		if (reg == 0)
			sx86_exec_full_modrmonly_ro_imm(ctx,w16,0,mod,rm,op_test8,op_test16,op_test32);
