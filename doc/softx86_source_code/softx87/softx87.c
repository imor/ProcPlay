...
/* all special cases seem to revolve around exp == 0x7FFF */
	if (ctx->state.st[i].exponent == 0x7FFF) {
		if (ctx->state.st[i].mantissa == 0) {
			if (ctx->state.st[i].sign_bit) {
				if (numtype)
...
/* 16383 is the IEEE-754 bias for double extended precision numbers */
	exp  = ctx->state.st[i].exponent - (16383+63);
	val  = (double)(ctx->state.st[i].mantissa);
...
/* log2 x = ln x / ln 2 */
	...
	if (val == 0) {
		ctx->state.st[i].sign_bit =	sgn;
		ctx->state.st[i].exponent =	0;
		ctx->state.st[i].mantissa =	0;
		return;
	}
...
/* store */
	ctx->state.st[i].sign_bit =		sgn;
	ctx->state.st[i].exponent =		l2b;
	ctx->state.st[i].mantissa =		(sx87_uldword)sal;
}
...
int softx87_init(softx87_ctx* ctx,int level)
{
	...
	ret=1;
	if (!ctx) return 0;
	if (level > SX87_FPULEVEL_80287) return 0; /* we currently support up to the 80287 */
