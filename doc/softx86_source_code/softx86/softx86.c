...
int softx86_reset(softx86_ctx* ctx)
{
	...
	if (ctx->level >= SX86_CPULEVEL_80286)
		ctx->addr_mask		= 0xFFFFFF;	/* 80286 24-bit addressing */
	else
		ctx->addr_mask		= 0xFFFFF;	/* 8086/8088/80186 20-bit addressing */

	if (ctx->level >= SX86_CPULEVEL_8086)	ctx->mem_addr_align = 2; /* 16-bit alignment for 8086, 80186, 80286 */
	else					ctx->mem_addr_align = 1; /* 8-bit alignment for 8088 */

	ctx->mem_addr_mask			= 0xFFFFFFFF << (ctx->mem_addr_align-1);
	...
	ctx->ptr_regs_8reg[0]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_AX].b.lo);	// AL
	ctx->ptr_regs_8reg[1]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_CX].b.lo);	// CL
	ctx->ptr_regs_8reg[2]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_DX].b.lo);	// DL
	ctx->ptr_regs_8reg[3]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_BX].b.lo);	// BL
	ctx->ptr_regs_8reg[4]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_AX].b.hi);	// AH
	ctx->ptr_regs_8reg[5]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_CX].b.hi);	// CH
	ctx->ptr_regs_8reg[6]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_DX].b.hi);	// DH
	ctx->ptr_regs_8reg[7]   = (sx86_ubyte*)(&ctx->state.general_reg[SX86_REG_BX].b.hi);	// BH
...
/* switch on/off bugs */
	ctx->bugs.preemptible_after_prefix =
		(ctx->level <= SX86_CPULEVEL_8086)?1:0;
	ctx->bugs.decrement_sp_before_store =
		(ctx->level <= SX86_CPULEVEL_80186)?1:0;
	ctx->bugs.mask_5bit_shiftcount =
		(ctx->level >= SX86_CPULEVEL_80286)?1:0;

/* opcode table */
	ctx->opcode_table		= (void*)(&optab8086);
...
/* sets instruction pointer to interrupt handler */
...
int softx86_step(softx86_ctx* ctx)
{
	...
	if (!ctx) return 0;
	sop = (Sfx86OpcodeTable*)(ctx->opcode_table);
	if (!sop) return 0;

	ctx->is_segment_override=0;
	ctx->segment_override=0;
	ctx->rep_flag=0;
...
//Just like the real thing, we break down every read/write into chunks of
//mem_addr_align bytes aligned on mem_addr_align addresses. For example, if
//emulating the 8086's 16-bit data/address bus, we never ask for more than 2 bytes
//at a time and we always stay on WORD boundaries.
///////////////////////
  sorry, this turns out to be an incorrect assumption based on the mis-interpretation
  of several technical diagrams. I assumed there were alignment issues because I didn't
  see any pin marked "address pin 0" on the diagram of a 286 when in fact there was.
  besides, if the above were true it would make a big mess out of known ways in which
  the CPU communicates with such bizarre memory schemes such as the EGA/VGA planar
  display memory.
  --------------------------------------------------------------------------------*/
...
void sx86_exec_full_modregrm_rw(softx86_ctx* ctx,sx86_ubyte w16,sx86_ubyte d32,sx86_ubyte mod,sx86_ubyte reg,sx86_ubyte rm,sx86_ubyte opswap,sx86_ubyte (*op8)(softx86_ctx* ctx,sx86_ubyte src,sx86_ubyte val),sx86_uword (*op16)(softx86_ctx* ctx,sx86_uword src,sx86_uword val),sx86_udword (*op32)(softx86_ctx* ctx,sx86_udword src,sx86_udword val))
{
	if (mod == 3) {		/* register <-> register */
		...
		if (w16) {
			...
			regv = *(ctx->ptr_regs_8reg[reg]);
			rmv = *(ctx->ptr_regs_8reg[rm]);

			if (opswap)
				*(ctx->ptr_regs_8reg[reg]) =
					op8(ctx,regv,rmv);
			else
				*(ctx->ptr_regs_8reg[rm]) =
					op8(ctx,rmv,regv);
		}
	}
	else {			/* register <-> memory */
...
*/
void sx86_exec_full_modrmonly_rw_imm(softx86_ctx* ctx,sx86_ubyte w16,sx86_ubyte d32,sx86_ubyte mod,sx86_ubyte rm,sx86_ubyte (*op8)(softx86_ctx* ctx,sx86_ubyte src,sx86_ubyte val),sx86_uword (*op16)(softx86_ctx* ctx,sx86_uword src,sx86_uword val),sx86_udword (*op32)(softx86_ctx* ctx,sx86_udword src,sx86_udword val))
{
	sx86_uword imm;
...
/* general purpose mod/rm executioneer helper for
   ...
   MOV WORD PTR [1334h],2222h
*/
void sx86_exec_full_modrmonly_ro_imm(softx86_ctx* ctx,sx86_ubyte w16,sx86_ubyte d32,sx86_ubyte mod,sx86_ubyte rm,sx86_ubyte (*op8)(softx86_ctx* ctx,sx86_ubyte src,sx86_ubyte val),sx86_uword (*op16)(softx86_ctx* ctx,sx86_uword src,sx86_uword val),sx86_udword (*op32)(softx86_ctx* ctx,sx86_udword src,sx86_udword val))
{
	sx86_uword imm;
...
void sx86_exec_full_modregrm_xchg(softx86_ctx* ctx,sx86_ubyte w16,sx86_ubyte d32,sx86_ubyte mod,sx86_ubyte reg,sx86_ubyte rm)
{
	if (mod == 3) {		/* register <-> register */
		...
		if (w16) {
			...
			tmp =					*(ctx->ptr_regs_8reg[reg]);
			*(ctx->ptr_regs_8reg[reg]) =		*(ctx->ptr_regs_8reg[rm]);
			*(ctx->ptr_regs_8reg[rm]) =		tmp;
		}
	}
	else {			/* register <-> memory */
		sx86_uword seg,ofs;
