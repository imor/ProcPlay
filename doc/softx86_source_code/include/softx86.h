...
#define SOFTX86_VERSION_SUBLO		29
...
/* host-to-CPU interaction */
	void			(*on_idle_cycle)	(/* softx86_ctx */ void* _ctx);								/* called so host application can do finer things during the execution of one instruction */
...
// other
	...
	int				mem_addr_align;		// alignment used
	sx86_udword			mem_addr_mask;		// mask from alignment
	int				level;			// which CPU are we emulating?
	sx86_ubyte*			ptr_regs_8reg[8];	// pointer table to 8-bit regs given reg value in opcode
	sx86_ubyte			is_segment_override;	// set if a segment override was encountered
	sx86_uword			segment_override;	// segment override
